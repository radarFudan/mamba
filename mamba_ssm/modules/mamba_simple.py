...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, mamba_cache=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            # print("xz data type:", xz.dtype)
            # print("cache_conv_state data type:", cache_conv_state.dtype)
            # print("cache_ssm_state data type:", cache_ssm_state.dtype)
            ...
            # TODO, no function from cache_conv_state to new convoluted x
            ...
            assert self.activation in ["silu", "swish"]
            # y, new_cache_ssm_state = selective_scan_fn(
            y, new_cache_ssm_state = selective_scan_ref(
                x,
                dt,
                A,
                B,
        ...
        # TODO, previous
        if new_cache_conv_state is not None:
            conv_state = new_cache_conv_state.detach() 
        else:
            print("new_cache_conv_state is None")
            conv_state = cache_conv_state
        if new_cache_ssm_state is not None:
            ssm_state = new_cache_ssm_state.detach()
        else:
            print("new_cache_ssm_state is None")
            ssm_state = cache_ssm_state 

        # zero
        # conv_state = cache_conv_state # Need find the right way to set it. 
