...
class SelectiveScanFn(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
                return_last_state=True, cache_ssm_state=None):
        if u.stride(-1) != 1:
            u = u.contiguous()
        if delta.stride(-1) != 1:
        ...
        if C.dim() == 3:
            ...
            ctx.squeeze_C = True
        if cache_ssm_state is not None:
            cache_ssm_state = cache_ssm_state.contiguous()
        out, x, *rest = selective_scan_cuda.fwd(u, delta, A, B, C, D, z, delta_bias, delta_softplus, cache_ssm_state)
        # TODO, should I get the new_cache_ssm_state here? or below? 
        ctx.delta_softplus = delta_softplus
        ctx.has_z = z is not None
        last_state = x[:, :, -1, 1::2]  # (batch, dim, dstate)
        if not ctx.has_z:
            ctx.save_for_backward(u, delta, A, B, C, D, delta_bias, x, cache_ssm_state)
            return out if not return_last_state else (out, last_state)
        else:
            ctx.save_for_backward(u, delta, A, B, C, D, z, delta_bias, x, out, cache_ssm_state)
            out_z = rest[0]
    ...
    def backward(ctx, dout, *args):
        if not ctx.has_z:
            u, delta, A, B, C, D, delta_bias, x, cache_ssm_state = ctx.saved_tensors
            z = None
            out = None
        else:
            u, delta, A, B, C, D, z, delta_bias, x, out, cache_ssm_state = ctx.saved_tensors
        if dout.stride(-1) != 1:
...
def selective_scan_fn(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    """
    return SelectiveScanFn.apply(u, delta, A, B, C, D, z, delta_bias, delta_softplus, return_last_state, cache_ssm_state)
...
def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    if cache_ssm_state is not None:
        assert cache_ssm_state.shape == (batch, dim, dstate), "In selective_scan_ref, cache_ssm_state is of shape{cache_ssm_state.shape}"
        x = cache_ssm_state
    else:
        raise NotImplementedError("cache_ssm_state is None")
        assert cache_ssm_state is None, "In selective_scan_ref, cache_ssm_state is None"
    ...
    ## Fast version, 
    # use associative_scan instead of for loop, as a simple patch. 
    # deltaA: B * D * L * N
    # deltaB_u: B * D * L * N
    ...
    assert C.dim() == 3, "C.dim() is {}".format(C.dim())
    y = torch.einsum('b d l n, b n l -> b d l', hs[:,:,1:,:], C)
    # print("y.shape:", y.shape)

    assert x.shape == hs[:,:,0,:].shape, "x and hs[:,:,0,:] should have the same shape"
    # assert x.to(deltaB_u.device) == hs[:,:,0,:], "The first hidden state should not be changed?"
    last_state = hs[:,:,-1,:]
    ...
    # print("x is of shape", x.shape) # B * D * dstate
    # print("last state is of shape", last_state.shape) # B * D * dstate
    # print("out is of shape", out.shape) # B * D * T
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                ...
                C_proj_bias=None, delta_softplus=True, cache_conv_state=None, cache_ssm_state=None, checkpoint_lvl=1):
        """
             xz: (batch, dim, seqlen)
        """
        ...
        conv1d_out, new_cache_conv_state = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias, None, True, cache_conv_state)
        # conv1d_out, _ = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias, None, True, cache_conv_state)
        ...
        if D is not None:
            D = D.contiguous()
        out, scan_intermediates, out_z, new_cache_ssm_state = selective_scan_cuda.fwd(
            conv1d_out, delta, A, B, C, D, z, delta_bias, delta_softplus, cache_ssm_state
        )
        ctx.delta_softplus = delta_softplus
        ...
        ctx.save_for_backward(xz, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight,
                              ...
                              A, B, C, D, delta_bias, scan_intermediates, out, cache_conv_state, cache_ssm_state)
        return F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias), new_cache_conv_state, new_cache_ssm_state
    ...
    def backward(ctx, dout):
        ...
        (xz, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight, delta_proj_weight, out_proj_weight,
         conv1d_out, delta, A, B, C, D, delta_bias, scan_intermediates, out, cache_conv_state, cache_ssm_state) = ctx.saved_tensors
        L = xz.shape[-1]
        delta_rank = delta_proj_weight.shape[1]
        d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
        x, z = xz.chunk(2, dim=1)
        ...
        dx, dconv1d_weight, dconv1d_bias = causal_conv1d_cuda.causal_conv1d_bwd(
            x, conv1d_weight, conv1d_bias, dconv1d_out, None, dx, True, cache_conv_state
        )
...
def mamba_inner_fn(
    ...
    cache_conv_state=None, cache_ssm_state=None, 
):
    return MambaInnerFn.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              out_proj_weight, out_proj_bias,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus, cache_conv_state, cache_ssm_state)
...
def mamba_inner_ref(
    ...
    # kernel, TODO, make it return new cache_conv_state
    # x = causal_conv1d_fn(x, weight=rearrange(conv1d_weight, "d 1 w -> d w"), bias=conv1d_bias, seq_idx=None, activation="silu", cache_conv_state=cache_conv_state)
    
    # pytorch
    x, new_cache_conv_state = causal_conv1d_ref(x, weight=rearrange(conv1d_weight, "d 1 w -> d w"), bias=conv1d_bias, activation="silu", cache_conv_state=cache_conv_state)
    ...
    # kernel, TODO, make it return new cache_ssm_state
    # y, new_cache_ssm_state = selective_scan_fn(x, delta, A, B, C, D, z=z, delta_bias=delta_bias, delta_softplus=True, return_last_state=True, cache_ssm_state=cache_ssm_state)
    
    # pytorch
    y, new_cache_ssm_state = selective_scan_ref(x, delta, A, B, C, D, z=z, delta_bias=delta_bias, delta_softplus=True, return_last_state=True, cache_ssm_state=cache_ssm_state)
    
    return F.linear(rearrange(y, "b d l -> b l d"), out_proj_weight, out_proj_bias), new_cache_conv_state, new_cache_ssm_state
